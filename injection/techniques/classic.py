...
class Classic(InjectorBase):

    inject_level = 5

    def __init__(self, request_obj, request_info):
        super(Classic, self).__init__(request_obj, request_info)
    ...
    def gen_calculate_cmd(self, separator, TAG, num1, num2):
        USE_BACKTICKS = eval(DefaultConfig.get_value('Classic', 'UseBackticks'))
        if USE_BACKTICKS: # using `
            calculate_cmd = "`expr " + str(num1) + " + " + str(num2) + "`"
            result = "{separator}echo {tag}{cmd}{tag}{tag}".format(separator=separator, tag=TAG, cmd=calculate_cmd)
        else:  
            calculate_cmd = "$((" + str(num1) + "+" + str(num2) + "))"
            result = "{separator}echo {tag}$(echo {cmd}{tag}){tag}".format(separator=separator, tag=TAG, cmd=calculate_cmd)
        return result
